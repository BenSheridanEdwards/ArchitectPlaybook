...
| [`/testing-audit`](testing-audit/SKILL.md)                     | ready | Audit a TypeScript and React project's tests against the Testing Library query priority ladder, the well-known React Testing Library pitfalls, and an explicit testing philosophy (behaviour over implementation, snapshots are a smell, semantic tokens over utility classes); static-first with optional `--with-run` enrichment from Vitest/Jest coverage data; never runs Playwright or Cypress; optionally generates an implementation plan ordered by philosophy impact. |
...
| [`/typescript-audit`](typescript-audit/SKILL.md)               | ready | Audit a TypeScript project's type discipline across compiler configuration, type quality in source, type system usage, and type safety at IO boundaries; static-first with optional `--with-run` enrichment from `tsc --noEmit`; tunable per-file thresholds for `any`/`as`/`!`; soft Graphify dependency for centrality-prioritised cleanup; optionally generates an implementation plan. |
...
### `/testing-audit`
Audits a TypeScript and React project's tests across four layers — test runner and tooling, query priority and selector hygiene, interaction and async patterns, test design and coverage — plus a Layer 0 snapshot. Anchored on the **Testing Library query priority ladder** (`getByRole` first, `getByTestId` last) and the well-known React Testing Library pitfalls (use `screen`, prefer `userEvent` over `fireEvent`, use `find*` for elements that may not be present yet, `query*` only for absence assertions, single assertion per `waitFor` with no side effects, never wrap `render` in `act`, never call `cleanup` manually). Carries an explicit **Testing Philosophy** section: (1) test behaviour, not implementation; (2) snapshots are a smell — small bounded ones are tolerated as `partial`, large or implementation-coupled ones are `violation`; (3) assert against semantic theme tokens (`bg-primary`), never raw utility classes (`bg-gray-100`) — the latter is a `violation`. Static-first with opt-in `--with-run` for Vitest/Jest coverage data; **never runs Playwright or Cypress** (those have side effects). Soft Graphify dependency for components-without-tests detection. Implementation plan ordered by philosophy impact: behaviour-vs-implementation drift first, snapshot smells next, utility-class assertions next.

### `/typescript-audit`
Audits a TypeScript project's type discipline across four layers — compiler configuration, type quality in source, type system usage, type safety at IO boundaries — plus a Layer 0 snapshot. Layer 1 grades the strict-family flags (`strict`, `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`, `useUnknownInCatchVariables`, `isolatedModules`, and friends); Layer 2 grades source-level type quality with tunable per-file thresholds for `any`, `as` (excluding `as const`), and `!` (defaults 3, 5, 3); Layer 3 grades type-system idioms (discriminated unions, branded ID types, `as const`, utility types, generic constraints, conditional-type depth ≤ 3); Layer 4 grades runtime validation at IO boundaries (network responses, `JSON.parse`, form data, URL params, storage reads, environment variables) and recognises zod, valibot, arktype, io-ts, yup, joi, superstruct, and runtypes as validators (plus framework-level layers like tRPC and GraphQL Code Generator). Static-first with opt-in `--with-run` running `tsc --noEmit` (genuinely read-only — no emit, no side effects). Soft Graphify dependency: when present, type-quality offenders are prioritised by graph centrality so god nodes get cleaned up first. Boundary-aware with `/linting-audit` (which owns `@typescript-eslint` plugin coverage) and `/error-handling-audit` (which owns catch-typing as it affects errors).
