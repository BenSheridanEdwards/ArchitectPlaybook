...
**💡 Pro tip**: Spin this up in its own Git worktree with `/worktree react` (or just `/worktree` to pick from a list) so you can run multiple audits in true parallel without any conflicts.
...
Print the snapshot's headline numbers (React version, framework, component counts, custom-hook count, design system) plus a short table mirroring the four layers with counts of present, partial, missing, and violation per layer. When layer 4 is skipped, show the row with a "skipped (React below 18)" note instead of counts. Highlight the top three violations and the most widely-consumed custom hooks (graph-aware when available). Then ask the single yes-or-no question. Do not proceed to phase 2 without an explicit affirmative.
