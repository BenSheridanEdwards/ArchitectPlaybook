...
If the command exits non-zero **because of lint findings**, that is the expected output and the JSON is still parseable. If the command exits non-zero **because of a configuration error**, record the failure and degrade run-dependent diagnostic snapshot fields (top-suppressed rules, total warnings/errors) and any run-required check to `partial`.
...
| `--with-run` set but lint command fails to start | Binary missing, configuration invalid, monorepo path resolution issue. | Record the failure and the captured stderr in metadata. Run-dependent checks degrade to `partial`. Continue with the static analysis. |
