...
Layer 0 is informational only and has no status. When neither `.claude/settings.json` nor `.claude/settings.local.json` exists, every Layer 3 settings check is emitted explicitly as `not-applicable`/`not-evaluated` with a null status; none is omitted. Layer 4's drift checks always run.
...
| Testing philosophy or approach documented | The instruction file states the testing convention: framework, query priority (Testing Library), behaviour-over-implementation, snapshot policy. Soft check. Explicitly not applicable when no test files are present. | Tests exist but no testing convention is documented. |
...
| Tone or response-style guidance present | When the project surfaces user-facing AI features (signal: dependencies on `ai`, `@ai-sdk/*`, `langchain`, `@anthropic-ai/sdk`, `openai`), the instruction file documents the desired response style or persona for those features. Soft check. Explicitly not applicable when no AI surface is detected. | AI surface present; no response-style guidance. |
| Imperative-present rule signal | When the project requires Conventional Commits, the instruction file or a referenced doc explicitly demands the imperative-present subject form ("Add X", not "Added X"). Soft check. Explicitly not applicable when Conventional Commits is not in use. | Conventional Commits required; imperative-present rule not stated. |

### Layer 3 — Claude Code settings hygiene

These checks read `.claude/settings.json` and, when present, `.claude/settings.local.json`. When neither exists, every check in the layer is emitted explicitly as `not-applicable`/`not-evaluated` with `status: null` and reason `claude-settings-not-adopted`. Settings checks **do not** rate the absence of the file as a violation; users who do not run Claude Code locally have no reason to commit a `settings.json`.
...
| MCP-server entries are auditable | When `mcpServers` is present, every entry has a recognisable command, a transport, and either no secrets or environment-variable references for secrets. Soft check. Explicitly not applicable when no `mcpServers` block exists. | A concrete secret in an MCP-server config. |
...
| No abbreviations in agentic prose (when project requires it) | When `CLAUDE.md` or another rule file states a "no abbreviations" policy, the instruction files themselves comply with that policy. Soft check. Explicitly not applicable when no abbreviations policy is declared. | The policy is declared and the file violates it. |
...
For each check in the active layer list, walk its detection logic. Emit Layer 3 checks as explicitly not applicable when no `.claude/settings*.json` file exists. Emit Layer 2's testing-philosophy check as not applicable when no test files are present, and its tone-and-response-style check as not applicable when no AI surface is detected. Every catalog check remains present exactly once.
  ...
  "checks": [
    {
      ...
      "checkId": "agentic-audit.no-secrets-in-settings-json",
...
| Project uses a non-Node ecosystem | Python or Rust project. | Emit Layer 4's `package.json`-script-drift check explicitly as `not-applicable`/`not-evaluated` with a null status. The other layers still run; tech-stack-mentions cross-referencing degrades to "no dependency manifest available" and reports `partial` rather than `violation` for stale mentions. |
