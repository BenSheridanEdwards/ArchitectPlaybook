...
5. **When `--with-run` is set**, invokes `npx tsc --noEmit` and parses the diagnostics. If `tsc` fails to start (not installed in the project), records the failure and continues with run-dependent enrichment degrading to `partial`.
...
| `--with-run` set but `tsc` fails to start | Binary missing, invalid configuration, monorepo path issue. | Record the failure and the captured stderr in metadata. Run-dependent enrichment of the diagnostic snapshot degrades to `partial`. The static analysis still runs. |
