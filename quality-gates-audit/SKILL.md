...
In the chat session, print a short table that mirrors the three stages, with a count of present, misconfigured, and missing per stage. Then ask the user the single yes-or-no question described above. Do not proceed to phase 2 without an explicit affirmative.
